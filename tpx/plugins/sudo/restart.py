...
import config
from tpx import app
from tpx.misc import HAPP, SUDOERS, XCB
...
async def restart_(_, message):
    response = await message.reply_text("Kr raha hu bsdk ruk ja 🫩")
    ac_chats = await get_active_chats()
    for x in ac_chats:
        try:
            await app.send_message(
                chat_id=int(x),
                text=f"{app.mention} ɪs ʀᴇsᴛᴀʀᴛɪɴɢ...\n\n 15-20 sᴇᴄᴏɴᴅs baad play Krna lodu song agar error nhi aaya restart code me🤬😮‍💨",
            )
            await remove_active_chat(x)
            await remove_active_video_chat(x)
        except:
    ...
    os.system(f"kill -9 {os.getpid()} && bash toxic")
